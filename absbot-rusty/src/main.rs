use chrono::{self, Local, LocalResult::Single, NaiveDateTime, NaiveTime, TimeZone};
...
fn main() {
    let (hour, minute, lesson_id) = match get_input() {
        Ok(res) => res,
        Err(e) => {
            end(e);
            return;
    ...
    let target =
        match chrono::Local.from_local_datetime(&NaiveDateTime::new(target_date, target_time)) {
            Single(res) => res.timestamp_millis() as u64 + 1,
            _ => {
                end("Something went wrong when converting time".into());
                return;
            }
        };
...
fn get_auth_token() -> Result<String, String> {
    ...
    let driver = WebDriver::new("http://localhost:4444", &caps)
        .map_err(|_| String::from("Could not find Webdriver"))?;
    driver
        .get("https://schalter.asvz.ch/")
        .map_err(|_| String::from("Could not connect to asvz's website"))?;
    loop {
    ...
    let key = driver
        ...
        .map_err(|_| String::from("Could not fetch the json key"))?
        .convert::<String>()
        .map_err(|_| String::from("Could not parse the json key"))?;

    let raw_json: String = driver
        .execute_script(&format!("return localStorage.getItem('{}')", key))
        .map_err(|_| String::from("Could not fetch the raw JSON data"))?
        .value()
        .as_str()
        .ok_or_else(|| String::from("The raw JSON is not in String format"))?
        .into();

    let parsed_json = serde_json::from_str::<serde_json::Value>(&raw_json)
        .map_err(|_| String::from("Could not parse the raw JSON"))?;
    let auth_token = parsed_json["access_token"]
        .as_str()
        .ok_or_else(|| String::from("No field 'access_token' in the JSON"))?;

    Ok(auth_token.into())
}

fn make_request(auth_token: String, lesson_id: u32, target: u64) -> Result<StatusCode, String> {
    let url = format!(
        "https://schalter.asvz.ch/tn-api/api/Lessons/{}/enroll??t={}",
        lesson_id, &target
    );
    let client = blocking::Client::new();
    let request = client.post(url).body("{}").headers(
        construct_headers(auth_token, lesson_id)
            .map_err(|_| String::from("Error creating headers"))?,
    );
    let response = request.send().expect("Something went wrong :(");
    Ok(response.status())
}

fn construct_headers(auth_token: String, lesson_id: u32) -> Result<HeaderMap, String> {
    let mut headers = HeaderMap::new();

    let auth = HeaderValue::from_str(&format!("Bearer {}", auth_token))
        .map_err(|_| String::from("Could not create Authorization header entry"))?;
    let referer = HeaderValue::from_str(&format!(
        "https://schalter.asvz.ch/tn/lessons/{}",
        lesson_id
    ))
    .map_err(|_| String::from("Could not create Referer header entry"))?;
...
fn get_input() -> Result<(u32, u32, u32), String> {
    ...
    io::stdin()
        .read_line(&mut hour)
        .map_err(|_| String::from("Error whilst reading input"))?;
    let hour = hour
        .trim()
        .parse::<u32>()
        .map_err(|_| String::from("Invalid entry for hour"))?;
    if hour > 23 {
        return Err("Hour not in specified interval".into());
    }
    ...
    let minute = minute
        .trim()
        .parse::<u32>()
        .map_err(|_| String::from("Invalid entry for minute"))?;
    if minute > 59 {
        return Err("Minute not in specified interval".into());
    }
    ...
    let lesson_id = lesson_id
        .trim()
        .parse::<u32>()
        .map_err(|_| String::from("Invalid entry for lesson_id"))?;
    if minute > 999999 {
        return Err("Lesson_id not in specified interval".into());
    }
    Ok((hour, minute, lesson_id))
}

fn end(mut msg: String) {
    println!("{}", &msg);
    println!();
